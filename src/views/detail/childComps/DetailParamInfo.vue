<template>
  <div class="param-info" v-if="Object.keys(paramInfo).length!==0">
    <table v-for="(table,index) in paramInfo.rule.tables" :key="index">
      <tr v-for="(item,indey) in table" :key="indey">
        <td v-for="(tds,indez) in item" :key="indez">{{tds}}</td>
      </tr>
    </table>
    <div class="param-disclaimer">{{paramInfo.rule.disclaimer}}</div>
